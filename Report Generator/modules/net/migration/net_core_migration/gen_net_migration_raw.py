...
FILE_TITLE = "Non_Compatible_Assembly_Method.csv"


class GenNetMigrationRaw(AbstractImagingService):
    """
    ...
    def create_record(self, node: Node) -> AssemblyDecommissionRecord:
        ...
        record.compatible_net_core = self.__raw_object_service.get_property_value(node, "Dotnet core", "")
        record.compatible_standard_plus = self.__raw_object_service.get_property_value(node, "Dotnet core plus platform extensions",  "")
        record.compatible_net_core_plus =  self.__raw_object_service.get_property_value(node, "Dotnet standard plus platform",  "")
    ...
    def __get_records(self):
        ...
        nodes = self.__net_metrics.get_net_artifacts()
        ...
        for x in nodes:
            try:
                ...
                self.__logger.warn("Failed to process node with id: %d".format(x.id))
    ...
    def __init__(self):
        ...
        """
        super(GenNetMigrationRaw, self).__init__()

        self.__logger = Logger.get_logger("Gen .Net Core Migration Raw")
